	...
	public void run(){
		...
		try{
			...
			if((client = (InetSocketAddress)FdMain.channel.receive(rvbuffer)) != null)
			{
				...
				ByteArrayInputStream bais = new ByteArrayInputStream(rvbufferarr);
				...
				if((rvobj = ois.readObject()) instanceof Map<?,?>)
				{
					...
					for(int i=0; i<4; i++)
					{
						...
						if(rvhc > cuhc)
						{
							FDList.get(5554+i).record(System.currentTimeMillis(), 0L);
					...
					{						
						...
						else
						{
							if(i != FdMain.SendportList.get(5502))
							{
								for(int j=0; j<4; j++)
								{
									if(suspectMtx[i][j] == -1 && rvsm[i][j] == 1)
										suspectMtx[i][j] = rvsm[i][j];
									else
										suspectMtx[i][j] = suspectMtx[i][j] | rvsm[i][j];
								}
							}
						}
					}					
				...
				if(adapFD != null)
				{
					if(! adapFD.shouldConvictPhi(now,1) ) {
						...
						for(int j=0; j<4; j++)
						{
							if(suspectMtx[j][i] == 1)
								dcount++;
							if((dcount>aliveList.size()/2 || aliveList.size() == 2) && suspectMtx[i][i] == 0)
							{
								suspectMtx[i][i] = 1;
								if(j>=i)
								{
							...
							if(dcount == aliveList.size())
							{
								if(fcflag[i] == 0)
								{
									...
									fcflag[i] = 1;
									fccount++;
									if(i == 0 && gossipList.get(5554) > 1000 * index)
									{
										if(index == 1)
										{
											index++;
								...
								System.out.println("5554+"+i+"-Dead Consensus-");
								System.out.println("Consensus time: "+fctime[i]);
